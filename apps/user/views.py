...
from dailyfresh2.sf import conn
...
def pie(request):
    df = pd.read_sql('SELECT * FROM df_goods_sku', con=conn)
    # # df1 = pd.read_sql('SELECT * FROM df_order_goods', con=conn)
    #
    # # print(df.sales)
    x_name = df["name"].tolist()
    # print(x_name)
    y_name = df["sales"].tolist()
    # print(y_name)
    a = (
        Pie()
            .add("", [list(z) for z in zip(x_name, y_name)],radius='65%')
            .set_global_opts(title_opts=opts.TitleOpts(title="销售额"), legend_opts=opts.LegendOpts(pos_left="80%"), )
            .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {c}"))
    )
    # return render_template("销售额饼状图.html", a_data=a.dump_options())
...
def sjfx(request):
    return render(request, 'sjfx.html')

def sjfx1(request):
    return render(request, 'sjfx1.html')
